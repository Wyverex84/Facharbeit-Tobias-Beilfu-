 ...
 * VERSION: 2.1<br>
 * LAST MODIFIED: February, 8, 2012<br>
 * <br>
 * DESCRIPTION<br>
 * Bar class for use as a health bar or progress bar as well as for other types of measurements for visual
 * of value in a bar form.<br>
 * <br>
 * DO NOT EDIT THIS CLASS as everything can be controlled through method calls.<br>
 */
...
//  The color of the bar AT the breakpoint will be the average color between the safe color and the danger color
    private int fontSize = 18; // the size of the text
    ...
    private void newImage()
    {
        ...
        GreenfootImage rightImg = (showTextualUnits) ?
            new GreenfootImage(" "+value+" "+unitOfMeasure, fontSize, textColor, backgroundColor) :
            new GreenfootImage(1, 1);
        int maxX = (leftImg.getWidth() > rightImg.getWidth()) ?
            leftImg.getWidth() :
            rightImg.getWidth();
        ...
        if (barValue > 0)
        {
            int transValue = breakValue;
            if (!usingBreakValue) transValue = breakPercent*(maximumValue-minimumValue)/100+minimumValue;
            if (breakLow) { if (value > transValue) barImg.setColor(safeColor); else barImg.setColor(dangerColor); }
            else { if (value < transValue) barImg.setColor(safeColor); else barImg.setColor(dangerColor); }
            if (value == transValue)
            {
                int r = (safeColor.getRed()+dangerColor.getRed())/2;
                int g = (safeColor.getGreen()+dangerColor.getGreen())/2;
                int b = (safeColor.getBlue()+dangerColor.getBlue())/2;
    ...
    {
        if (brkVal < minimumValue || brkVal > maximumValue) return;
        breakValue = brkVal;
