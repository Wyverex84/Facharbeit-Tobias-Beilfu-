...
{
    private boolean countDownward = true; // flag determining direction of count (up or down)
    private boolean countingFrames = true; // flag determining what is counted (frames or nilliseconds)
    private int delayCount = 100; // the number of frames or milliseconds that pass before the value of the bar changes
    private int count = 0; // the current number of frames or milliseconds that have passed since the last value change
    private boolean countExpired = false; // flag indicating the the counter had run its course
    private boolean isRunning = false; // flag indicating whether the counter is currently counting or not
    private long startTime = System.currentTimeMillis(); // used in getting the actual passage of time in milliseconds
    ...
    public void act()
    {
        if (countExpired || !isRunning) return;
        if (countingFrames) count++;
        else
        {
        ...
        if (count >= delayCount)
        {
            ...
            if ((countDownward && getValue() == 0) || (!countDownward && getValue() == getMaximumValue())) countExpired = true;
            count = 0;
        }
    }
     ...
     */
    public void restart() { if (!countExpired) isRunning = true; }
     ...
     */
    public boolean isExpired() { return countExpired; }
     ...
     */
    public boolean getCountDownward() { return countDownward; }
     ...
     */
    public void setCountDownward(boolean ctDownward) { if (!isRunning) countDownward = ctDownward; }
     ...
     */
    public void setDelayCount(int delayCt) { if (!isRunning) delayCount = delayCt; }    
}
