...
public abstract class PhysicalActor extends AdvancedActor {
    ...
    protected void collisions(){
        ...
        if(collider.host instanceof PhysicalActor){
            ...
            Vector2D vCollisionNorm = new Vector2D(hit.getLocation()).add(getLocation().inverted()).normalized();
            ...
            double speed = Vector2D.dotProduct(vRelativeVelocity, vCollisionNorm);
    ...
    protected static Vector2D reflect(Vector2D base, Vector2D reflect, double bounce){
        ...
        out.add(Vector2D.project(reflected, base.orthogonal()).scale(bounce));
        return out;
    }
    ...
    public void addAccAt(Vector2D forceLocation, Vector2D acceleration){
        ...
        Vector2D spinMovement = Vector2D.project(restForce, applyLocToCenter.orthogonal());
