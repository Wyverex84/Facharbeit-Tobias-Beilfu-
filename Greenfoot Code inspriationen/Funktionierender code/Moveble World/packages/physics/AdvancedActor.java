 ...
 * @see Collider
 * @see packages.twoD.Vector
 * @see packages.twoD.Transform
 */
public abstract class AdvancedActor extends Actor{
    ...
    /**
     * This method is called by the Greenfoot system when this actor has been inserted into the world. This method can be overridden to implement custom
     * behaviour when the actor is inserted into the world. This replaces the {@code addedToWorld(greenfoot.World)} method.
     * <p>The default implementation does nothing.
     * 
     * @param w The world the actor was added into
     */
    ...
    private void internalAct(){
        if(getCollider() != null){
            ...
            if(Greenfoot.mousePressed(this)) onPress();
            else if(Greenfoot.mouseClicked(this)) onRelease();
        }
    }
    ...
    public void fixedMove(Vector movement){
        movement = new Vector(movement).scale(time.deltaTime());
        move(movement);
    }
    ...
    public void fixedMove(double distance){
        fixedMove(Vector.angledVector(transform.rotation, distance));
    }
     ...
     */
    public void fixedMove(Transform movement){
        movement = new Transform(movement);
    ...
    public void fixedMoveX(double distance){
        fixedMove(Vector.angledVector(0, distance));
    }
    ...
    public void fixedMoveY(double distance){
        fixedMove(Vector.angledVector(90, distance));
    }
     ...
     */
    public Vector fixedMoveInSteps(Vector movement, double stepSize){
        movement = new Vector(movement).scale(time.deltaTime());
    ...
    public double fixedMoveInSteps(double distance, double stepSize){
        return fixedMoveInSteps(Vector.angledVector(transform.rotation, distance), stepSize).abs();
    }
    ...
    public void move(double distance){
        move(Vector.angledVector(transform.rotation, distance));
    }
     ...
     */
    public void move(Transform movement){
    ...
    public void moveX(double distance){
        move(Vector.angledVector(0, distance));
    }
    ...
    public void moveY(double distance){
        move(Vector.angledVector(90, distance));
    }
     ...
     */
    public Vector moveInSteps(Vector movement, double stepSize){
        for(double i=0; i<movement.abs()-stepSize; i+= stepSize){
            move(movement.normalized().scale(stepSize));
            if(!onStep((int)(i / stepSize))) return Vector.angledVector(movement.angle(), i);
        }
        move(movement.normalized().scale(movement.abs() % stepSize));
        if(!onStep((int)(movement.abs() / stepSize))) return Vector.angledVector(movement.angle(), (int)movement.abs());
        return movement;
    }
     ...
     * @return
     */
    public Vector moveInSteps(double distance, double stepSize){
        return moveInSteps(Vector.angledVector(transform.rotation, distance), stepSize);
    }
    ...
    public void setX(double x){
        setLocation(new Vector(x, transform.location.x));
    }
    ...
    public void setY(double y){
        setLocation(new Vector(transform.location.y, y));
    }
    ...
    /**
     * Moves the actor to the specified location(the location the vector points at starting at the point of origin).
     * 
    ...
    public Vector getLocation(){
        return new Vector(transform.location);
    }
