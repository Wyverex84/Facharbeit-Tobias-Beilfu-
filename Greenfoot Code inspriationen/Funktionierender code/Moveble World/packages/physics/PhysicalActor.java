...
import packages.twoD.Transform;
...
public abstract class PhysicalActor extends AdvancedActor{
     ...
     */
    public static final Vector GRAVITY = new Vector(0, 9.81);
    ...
    protected void applyAirDrag(){
        addFixedForce(Vector.angledVector(getMovementDirection() + 180, cw * CW_SCALE * getSpeed() * getSpeed()));
    }
    ...
    protected void applyGroundDrag(){
        ...
        fixedAccelerate(Vector.angledVector(getMovementDirection() + 180, drag));
    }
     ...
     * @see #SLIDE_DELAY
     */
	public static double gripLevel(Vector slide){
    ...
    protected void inertia(){
        fixedMove(new Transform(movement).scale(UNIT));
    }
    ...
    protected void collisions(){
        ...
        if(collider.host instanceof PhysicalActor){
            ...
            Vector vCollisionNorm = new Vector(hit.getLocation()).add(getLocation().inverted()).normalized();
            ...
            Vector vRelativeVelocity = new Vector(velocity()).add(hit.velocity().inverted());
            
            double speed = Vector.dotProduct(vRelativeVelocity, vCollisionNorm);
            
            if(speed < 0) return;
            
            double impuls = (1 + (bounce + hit.bounce) / 2) * speed / (mass() + hit.mass());
            
            movement.location.add(new Vector(vCollisionNorm).scale(-impuls * hit.mass()));
            hit.movement.location.add(new Vector(vCollisionNorm).scale(impuls * mass()));
        }
        else{
            Vector edge = collider.getEdgeTowards(getCollider());
            for(int i=0; i<100&&collider.intersects(getCollider()); i++)
                move(movement.location.inverted().normalized());
     ...
     */
    protected static Vector reflect(Vector base, Vector reflect, double bounce){
        Vector reflected = Vector.reflect(base, reflect);
        Vector out = Vector.project(reflected, base);
        out.add(Vector.project(reflected, base.orthogonal()).scale(bounce));
        return out;
    }
    ...
    private void worldCollisions(){
        ...
        if(getCollider() != null && getCollider().getWorldEdge() != null){
            Vector edge = getCollider().getWorldEdge();
            movement.location = reflect(edge, movement.location, bounce);
            for(int i=0; i<100&&edge.equals(getCollider().getWorldEdge()); i++){
                move(movement.location.normalized());
                if(i == 99) move(movement.location.normalized().invert().scale(100));
            }
        }
    }
     ...
     * @see #movement
     */
    public void setMovement(Transform newMovement){
        if(newMovement == null) return;
        if(newMovement.location == null) newMovement.location = getMovement().location;

        if(Double.isNaN(newMovement.location.x)) newMovement.location.x = getMovement().location.x;
        if(Double.isNaN(newMovement.location.y)) newMovement.location.y = getMovement().location.y;
     ...
     * @see #setMovement(Transform)
     */
    public void setVelocity(Vector newVelocity){
        setMovement(new Transform(newVelocity, getAngle()));
    }
    ...
    public void setSpin(double newRotationalSpeed){
        setMovement(new Transform(velocity(), newRotationalSpeed));
    }
     ...
     * @see #setMovement(Transform)
     */
    public void addMovement(Transform addedMovement){
        Transform newMovement = getMovement();
     ...
     * @see #addMovement(Transform)
     */
    public void addVelocity(Vector addedVelocity){
        addMovement(new Transform(addedVelocity));
    }
    ...
    public void addSpin(double addedRotationalSpeed){
        addMovement(new Transform(addedRotationalSpeed));
    }
     ...
     * @see #worldLocOf(Vector)
     * @see #addForce(Vector)
     * @see #addAccAt(Vector, Vector)
     */
    public void addForceAt(Vector forceLocation, Vector force){
     ...
     * @see #worldLocOf(Vector)
     */
    public void addAccAt(Vector forceLocation, Vector acceleration){
        Vector overallAcc = acceleration;
        Vector applyLocToCenter = Vector.between(forceLocation, getLocation());
        ...
        Vector directlyAppliedForce = overallAcc.scaled(directlyAppliedForcePercentage);
        Vector restForce = overallAcc.scaled(restForcePercentage);


        Vector additionalMovement = Vector.project(restForce, applyLocToCenter);
        Vector spinMovement = Vector.project(restForce, applyLocToCenter.orthogonal());


        Vector spinTarget = forceLocation.added(spinMovement);
        double spin = Vector.angleBetween(Vector.between(spinTarget, getLocation()), applyLocToCenter);
     ...
     * @see #addMovement(Transform)
     */
    public void addFixedMovement(Transform addedMovement){
        Transform newMovement = getMovement();
        newMovement.add(new Transform(addedMovement).scale(time.deltaTime()));
        setMovement(newMovement);
    }
     ...
     * 
     * @param addedVelocity The velocity that should be added
     * @see #adFixeddMovement(Transform)
     */
    public void addFixedVelocity(Vector addedVelocity){
        addFixedMovement(new Transform(addedVelocity));
    }
     ...
     * 
     * @param addedRotationalSpeed The rotational speed that should be added(in degrees per second)
     * @see #addFixedMovement(Transform)
     */
    public void addFixedSpin(double addedRotationalSpeed){
        addFixedMovement(new Transform(addedRotationalSpeed));
    }
     ...
     * 
     * @param addedForce The force that should be added
     * @see #addFixedVelocity(Vector)
     */
    public void addFixedForce(Vector addedForce){
     ...
     * @param force The force to put onto the object
     * @see #worldLocOf(Vector)
     * @see #addFixedForce(Vector)
     * @see #addForceAt(Vector, Vector)
     */
    public void addFixedForceAt(Vector forceLocation, Vector force){
     ...
     * @see #addFixedVelocity(Vector)
     */
    public void fixedAccelerate(Vector acceleration){
     ...
     * @param acceleration The acceleration for the object
     * @see #addAccAt(Vector)
     * @see #fixedAccelerate(Vector)
     */
    public void addFixedAccAt(Vector loc, Vector overallAcc){
    ...
    public Vector momentum(){
        return new Vector(movement.location).scale(mass());
    }
    ...
    public Vector velocity(){
        return new Vector(movement.location);
    }
     ...
     */
    public Vector worldLocOf(Vector relativeLoc){
        return getLocation().add(Vector.angledVector(getAngle() + relativeLoc.angle(), relativeLoc.abs()));
    }
